...
NS_IMETHODIMP nsNativeEvents::MouseMove(nsISupports *aNode, PRInt32 startX, PRInt32 startY, PRInt32 endX, PRInt32 endY, PRInt32 duration)
{
  AccessibleDocumentWrapper doc(aNode);
  ...
  WD_RESULT res = mouseMoveTo(windowHandle, duration, startX, startY, endX, endY);

  return res == SUCCESS ? NS_OK : NS_ERROR_FAILURE;
}

/* void click (in nsISupports aNode, in long button, in long x, in long y); */
...
NS_IMETHODIMP nsNativeEvents::MouseRelease(nsISupports *aNode, PRInt32 x, PRInt32 y, PRInt32 button)
{
  ...
  LOG(DEBUG) << "Calling mouseDownAt: " << x << ", " << y;
...
static const mozilla::Module::CIDEntry kEventCIDs[] = {
    { &kEVENTS_CID, false, NULL, nsNativeEventsConstructor },
    { NULL }
};

static const mozilla::Module::ContractIDEntry kEventContracts[] = {
    { EVENTS_CONTRACTID, &kEVENTS_CID },
    { NULL }
};

static const mozilla::Module kEventModule = {
    mozilla::Module::kVersion,
    kEventCIDs,
    kEventContracts
};

NSMODULE_DEFN(nsEventModule) = &kEventModule;
NS_IMPL_MOZILLA192_NSGETMODULE(&kEventModule)
