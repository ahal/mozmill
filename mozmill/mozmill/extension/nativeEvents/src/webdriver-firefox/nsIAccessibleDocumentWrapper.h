...
public:
	...
	{	
		...
		wrapper = do_QueryInterface(node);
	...
	void* getWindowHandle() 
	{
		...
		if (wrapper){
			rv = wrapper->GetWindowHandle(&handle);
			if(NS_SUCCEEDED(rv)){ return handle; }
		}
...
private:
	bool isValid() const 
	{
		return (wrapper_18 != NULL || wrapper_19 != NULL || wrapper != NULL);
	}

	nsCOMPtr<nsIAccessibleDocument_18> wrapper_18;
	nsCOMPtr<nsIAccessibleDocument_19> wrapper_19;
	nsCOMPtr<nsIAccessibleDocument> wrapper;
};
