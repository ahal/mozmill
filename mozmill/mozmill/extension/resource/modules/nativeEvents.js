...
var EXPORTED_SYMBOLS = ["events", "Utils", "click",];
...
function click() {
  ...
  var sendClick = lib.declare("add", ctypes.default_abi, ctypes.int32_t, ctypes.int32_t, ctypes.int32_t);
  dump(sendClick(7, 3) + "\n");
}
...
/**
 * Click on element at location (x,y) with specified mouse button 
 * (1=primary, 2=middle, 3=secondary)
 */
events.synthesizeMouse = function(element, offsetX, offsetY, modifiers) {
  var clientRect = element.getBoundingClientRect();

  // Ensure we're not trying to click outside the element boundaries
  if (offsetX >= clientRect.width || offsetY >= clientRect.height || offsetX < 0 || offsetY < 0) {
    frame.events.fail({'function':'webdriver.events.synthesizeMouse()', 'message':'Specified position is outside of elements boundary'});
  }
  
  var x = clientRect.left + offsetX;
  var y = clientRect.top + offsetY;
  var clickCount = modifiers.clickCount || 1;
  var eventType = modifiers.type || 'click';
  var button = modifiers.button || 0;
  
  // The controller API uses '0' as the left mouse button but GDK uses '1'
  button += 1;
  

  // Load native events CPP module  
  var event = Utils.getNativeEvents();
  element = Utils.getNodeForNativeEvents(element);
  
  switch(eventType){
    case 'click':
      for(var i = 0; i < clickCount; ++i){
        event.click(element, x, y, button);
      } 
      break;
    case 'mouseDown':
      event.mousePress(element, x, y, button);
      break;
    case 'mouseUp':
      event.mouseRelease(element, x, y, button);
      break;
    case 'mouseMove':
      throw new Error("Native mouse move events not implemented yet");
    default:
...
/**
 * Send specified text to an element
 */
events.synthesizeKeys = function(element, text) {
  var event = Utils.getNativeEvents();
  element.focus();
  element = Utils.getNodeForNativeEvents(element);
  event.sendKeys(element, text);
  //frame.events.fail({'function':'webdriver.events.native_click()', 'message':'Event: ' + event + ', Element: ' + element})
}


/** 
 * Utils Module from Webdriver
 *
 * Mostly unused by us, but leaving it here for now as it
 * may be needed later
 *
 **/
function Utils() {
